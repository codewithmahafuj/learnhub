...
- Testing users/sessions: JWT cookies survive user deletion; sign out via GET `/api/auth/signout` then clicking the form button (direct POST hits MissingCSRF). `curl -c jar` + `/api/auth/csrf` + POST `/api/auth/callback/credentials` works for scripted auth tests.
